...
class GovernanceObject(BaseModel):
    ...
    # sync idapayd gobject list with our local relational DB backend
    @classmethod
    def sync(self, idapayd):
        golist = idapayd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_idapayd(idapayd, item)
    ...
    @classmethod
    def import_gobject_from_idapayd(self, idapayd, rec):
        import decimal
        import idapaylib
        import inflection
        ...
        # shim/idapayd conversion
        object_hex = idapaylib.SHIM_deserialise_from_idapayd(object_hex)
        objects = idapaylib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from idapayd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from idapayd, with every run
        ...
        # get/create, then sync payment amounts, etc. from idapayd - Idapayd is the master
        try:
            ...
            if sub.is_valid(*params) is False:
                govobj.vote_delete(idapayd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from idapayd! %s" % e)
            govobj.vote_delete(idapayd)
            return (govobj, None)
    ...
    def vote_delete(self, idapayd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(idapayd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, idapayd, signal, outcome):
        import idapaylib
        ...
        output = idapayd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = idapaylib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(idapayd, signal)

    def sync_network_vote(self, idapayd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = idapayd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = IDAPAYD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 idapay addr, non-multisig
            if not idapaylib.is_valid_idapay_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Idapay address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (idapaylib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/IdapayDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import idapaylib
        obj_data = idapaylib.SHIM_serialise_for_idapayd(self.serialise())
    ...
    def prepare(self, idapayd):
        try:
            object_hash = idapayd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = IDAPAYD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not idapaylib.is_valid_idapay_address(addr, config.network):
    ...
    def hash(self):
        import idapaylib
        return idapaylib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = IDAPAYD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, idapayd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - idapayd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, idapayd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - idapayd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, idapayd):
        now = int(time.time())
        return (self.created_at < (now - idapayd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, idapayd):
        if self.is_expired(idapayd):
            return False

        return True

    def is_deletable(self, idapayd):
        if self.is_expired(idapayd):
            return True
