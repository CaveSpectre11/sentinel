...
def is_valid_idapay_address(address, network='mainnet'):
    ...
    idapay_version = 140 if network == 'testnet' else 70
    ...
    try:
        ...
        # rescue from exception, not a valid Idapay address
        return False

    if (address_version != idapay_version):
        return False
...
# shims 'til we can fix the idapayd side
def SHIM_serialise_for_idapayd(sentinel_hex):
    from models import IDAPAYD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = IDAPAYD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in idapayd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # idapayd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    idapayd_hex = serialise(obj)
    return idapayd_hex


# shims 'til we can fix the idapayd side
def SHIM_deserialise_from_idapayd(idapayd_hex):
    from models import IDAPAYD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(idapayd_hex)
...
def did_we_vote(output):
    ...
    _, config_filename = os.path.split(config.idapay_conf)
    try:
