...
def has_idapay_conf():
    ...
    valid_idapay_conf = False

    # ensure idapay_conf exists & readable
    #
    # if not, print a message stating that Idapay Core must be installed and
    # configured, including JSONRPC access in idapay.conf
    try:
        f = io.open(config.idapay_conf)
        valid_idapay_conf = True
    except IOError as e:
        print(e)

    return valid_idapay_conf
...
def main():
    ...
    if not has_idapay_conf():
        print("IdapayCore must be installed and configured, including JSONRPC access in idapay.conf")
        sys.exit(1)
