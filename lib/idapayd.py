...
class IdapayDaemon():
    def __init__(self, **kwargs):
        host = config.sentinel_cfg.get('idapay_host', None)
        if host is None:
        ...
        # memoize calls to some idapayd methods
    ...
    @classmethod
    def from_idapay_conf(self, idapay_dot_conf):
        from idapay_config import IdapayConfig
        config_text = IdapayConfig.slurp_config_file(idapay_dot_conf)
        creds = IdapayConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = idapaylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = idapaylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = idapaylib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_idapayd_proto_version_with_sentinel_ping)
